...
| ID | Name | In plain English | Financial services example | Why it happens | Key control |
|---|---|---|---|---|---| 
| LLM01 | Prompt Injection | Prompt injection happens when user input alter the LLM application behaviour | In a financial service example a email that has malicious system prompts that can alter a fraud risk detection rules | It can happen when a user passes the bad prompt or via indect prompts such as malicious emal that can over ride system prompts or pdf's etc., | Treat external content as untrusted and isolate it from instructions., structured output validation and business rule enforcement outside the model, tool access should be scoped through a broker/API layer, not direct broad access, HITL for High-risk transactions, alert on priority downgrades, evidence omission, tool calls, policy override language, and unusual ticket changes |
| LLM02 | Sensitive Information Disclosure | The LLM application will provide sensitive information that the user or system doens't intended to have access too | A customer seervice chatbot have access to transaction history, SSN, PII information, risk score of the profile, fraud alerts on the profile | Excessive data, weak authorization, no field-level filtering, no output filtering, prompt contains secrets, training data issue, logging issue, multi-tenant isolation issue | Data minimiaztion, Implement RBAC/ABAC and scoped to purpose, content filtering to avoid sensitive data being passed or logged, meta data pre-tagging to the user specific session and post meta-data tagging validation before retriving |
| LLM03 | Supply Chain | In traditional web applications suppluy chain weaknesses comes from dependecies, base images, containers, CI/CD or 3rd party API's but in LLM it increases the scope to base models, training data, model poisioning, Lora adapters. The core concept is for every component in the LLM architecture, can we prove where it came from, who changed it, whether it was approved, whether it is safe to use, and whether it still behaves as expected |You download a highly rated open-source financial reasoning model from a public model repository. The model card is incomplete, the license is unclear, and no one verifies the model hash or tests it for backdoor behavior | It happens when you use a open source component or model with out proper governance and checks | Inventory -> Provence -> Integrity -> Approval testing -> monitoring |
| LLM04 | Data and Model Poisoning | Data/Model poisoning happens when model or training data, retraining data, embeddings data, fine tuning data is corrupted to alter model's intended behavior | If the organization have a customer support feedback chatbot and taking feedback, attackers are slow poisioning the model by providing continuous incorrect feedback and this data can be used in re-training the model with out proper data validation, it may change the Model's behavior over time | It may happen because of improper data sources, data validation controls, model and traning controls, RAG/vector database controls and Runtime controls  | It can be prevented by placing appropriate controls from sourcing to all the to the feedback loops such as source allowlist, data validation and lineage, model registry, backdoor testing, model trsuted ranking, auditiong, output validation etc., |
